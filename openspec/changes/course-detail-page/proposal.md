...
- **新增课程详情 API**：`GET /api/v1/training/courses/{course_id}` 返回课程完整信息（含教师、教室）
...
- **数据库迁移**：为 `room_follows` 表添加 `follow_type` 列

## Capabilities

### New Capabilities

- `course-detail-api`: 课程详情后端 API，返回课程完整信息（基本信息、教师、教室、标签等）
...
- **br-server**：
  ...
  - 新增 Schema：`br-server/app/schemas/course.py` 添加 `CourseDetailResponse`
  ...
  - 数据库迁移：新增 alembic 迁移文件
  - 测试：新增课程详情 API 测试和课程关注测试
...
- **br-app**：
  ...
  - 关注服务扩展：`br-app/src/services/followedRooms.js` 或新建 `followedCourses.js`

- **数据库**：`room_follows` 表新增 `follow_type` 列（默认值 `room`，向后兼容）

## 回滚方案

- 后端：`follow_type` 字段设默认值 `room`，迁移可安全回滚（downgrade 删除列）
