...
- `提交前只暂存本次改动文件`、`Dashboard 使用 Ant Design`：如果 packet 提供了一条可信且有后续复用价值的观察，可以创建 project 候选；候选不是已激活的长期记忆，Runtime 必须等待不同 change 的第二次独立成功证据后再晋级。技术专有名词保留原文，标题、理由和标签仍使用配置语言。
