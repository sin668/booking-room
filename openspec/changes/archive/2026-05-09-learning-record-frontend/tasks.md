...
- [x] 1.1 创建 `br-server/app/schemas/study_record.py`，参照 `br-server/app/schemas/booking.py` 的 Pydantic BaseModel 风格，定义以下 schema：
...
- [x] 2.1 创建 `br-server/app/services/study_record_service.py`，参照 `br-server/app/services/booking_service.py` 的函数式风格（非类），实现 `get_monthly_summary(db: AsyncSession, user_id: uuid.UUID, month: date)` 函数：
...
- [x] 2.2 在同一文件中实现 `list_study_records(db: AsyncSession, user_id: uuid.UUID, page: int, page_size: int, month: date | None)` 函数：
...
- [x] 3.1 创建 `br-server/app/api/routes/study_record.py`，参照 `br-server/app/api/routes/booking.py` 的路由风格：
...
- [x] 3.2 在 `br-server/app/main.py` 中注册路由：
...
- [x] 4.1 创建 `br-server/tests/test_study_record_service.py`，编写 service 层单元测试，覆盖以下场景：
  - 有 completed 记录的月份 → 正确返回统计和日历标记
  - 无 completed 记录的月份 → 统计为 0，calendar_mark 为空
  - 连续天数计算（跨周末等场景）
  - 分页列表查询正确性和按月筛选

- [x] 4.2 创建 `br-server/tests/test_api_study_record.py`，参照 `br-server/tests/test_api_booking.py` 的集成测试风格（httpx AsyncClient + seed fixture），覆盖：
  - summary 端点正常返回 200
  - list 端点正常返回 200 及分页
  - 未认证请求返回 401
...
- [x] 5.1 创建 `br-app/src/api/studyRecords.js`，参照 `br-app/src/api/bookings.js` 的风格：
...
- [x] 6.1 在 `br-app/src/pages.json` 的 pages 数组中添加路由注册：
  ```json
  {
...
- [x] 6.2 创建 `br-app/src/pages/study-record/index.vue`，参照 `prototype/study-record.html` 高保真原型和现有页面风格（如 `br-app/src/pages/profile/index.vue` 的 SCSS 变量体系），实现完整页面：
...
- [x] 7.1 更新 `docs/api.md`，参照现有 API 文档格式，补充学习记录两个接口的文档（请求参数、响应格式、错误码）

- [x] 7.2 代码审查：检查后端分层是否正确（routes → services → schemas）、前端是否遵循项目组件风格、有无重复代码
