...
## Corpus Check
- Large corpus: 1035 files · ~541,918 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 5390 nodes · 11428 edges · 210 communities detected
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 2524 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: contains: 2762 · uses: 2524 · ON_BRANCH: 1279 · MODIFIES: 1243 · calls: 1006 · method: 680 · rationale_for: 607 · PARENT_OF: 407 · imports_from: 324 · inherits: 306 · imports: 283 · re_exports: 7
...
- Excluded: 0 untracked · 89175 ignored · 0 sensitive · 24 missing committed
- Recommendation: Use --scope all or graphify.yaml inputs.corpus for a knowledge-base folder.

## Graph Freshness
- Built from Git commit: `8f9eaeb`
...
Cohesion: 0.06
Nodes (63): 2365213 fix: 修复admin取消pending_confirm订单时MissingGreenlet错误 - 移除admin_cancel_booking中不必要的with_for_update()行锁 - WalletTransaction字段改为构造函数直接传参替代setattr, 3486c32 docs: bug-fixed.md 新增 BUG-26 Admin取消待确认订单MissingGreenlet错误, 408d499 fix: admin_confirm_booking统一使用admin_get_booking重新查询构建响应 - 与admin_cancel_booking相同模式避免flush后MissingGreenlet, 5ec9d70 feat: course_schedules新增schedule_type字段 + 1V1定制排课延迟到确认时创建 - Model: CourseSchedule新增schedule_type(fixed/custom)字段 - Migration: 添加schedule_type列(server_default=fixed) - create_course_booking: 1V1定制不再写入排课表，仅将用户选择的日期/时间段存入booking记录 - admin_confirm_booking: 确认1V1定制时才创建schedule_type=custom的排课记录和课时记录 - admin_confirm_booking: 查询固定班课开始日期时过滤schedule_type=fixed, 8bd8340 feat: 1V1私人定制待确认订单流程 + Admin订单列表UI改造 - 后端: 1V1定制创建时设为pending_confirm状态 - 后端: 新增admin确认API(根据时间变更为pending或confirmed) - 后端: pending_confirm订单取消支持全额退款 - 后端: admin列表返回user_nickname替代user_id - Admin: 状态文案更新(已确认→进行中/pending→待开始) - Admin: 新增待确认状态，确认/取消按钮 - Admin: 去掉已确认订单取消按钮，pending订单增加取消按钮 - App: pending_confirm订单显示在待开始TAB - App: 新增pending_confirm状态样式(橙色主题), 986f1f0 fix: admin_cancel_booking统一使用admin_get_booking重新查询构建响应 - 避免flush后ORM对象状态导致MissingGreenlet错误, a9ce239 fix: 修复alembic迁移链循环依赖 - 将schedule_type迁移的revision ID从重复的a1b2c3d4e5f6改为唯一的e8f9a0b1c2d3, f174e70 fix: admin取消pending_confirm订单时全额退款不扣手续费 - admin_cancel_booking增加pending_confirm状态专属处理分支 - 绕过通用cancel_booking的confirmed状态限制 - 全额退款+恢复优惠券+创建钱包退款记录 (+55 more)
...
Cohesion: 0.06
Nodes (17): 8f9eaeb feat: bookings新增time_slots/teacher_id字段+确认订单按预约日期判断状态+排课列表显示类型+确认/取消按钮直接展示 - Booking模型+迁移b9c0d1e2f3a4: 新增time_slots(JSON)和teacher_id，对应course_schedules同名字段 - create_course_booking: 1V1定制保存用户所选时间段(时间槽)及排课授课老师到订单 - admin_confirm_booking: 当前日期>=预约日期→进行中，当前日期<预约日期→待开始 - _create_custom_schedule_on_confirm: 定制排课记录使用订单的time_slots和teacher_id - CourseScheduleResponse新增schedule_type，排课管理列表新增类型列(固定班课/定制课时) - 预约列表确认/取消按钮从更多下拉菜单改为直接展示, CourseCreateParams, CourseDetail, CourseItem, CourseListResult, CourseScheduleItem, CourseUpdateParams, LessonCreateParams (+9 more)
...
## Knowledge Gaps
- **427 isolated node(s):** `CourseScheduleItem`, `CourseListResult`, `CourseCreateParams`, `CourseUpdateParams`, `TeacherBrief` (+422 more)
...
- **Thin community `Booking Cancellation Tests`** (1 nodes): `TestCreateBooking`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Room & Seat Management`** (1 nodes): `TestCancelBooking`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Room Management Tests`** (2 nodes): `Integration tests for file upload API endpoints.`, `TestUploadAPI`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Booking Service Tests`** (1 nodes): `AdminMenuService`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Wallet Service Layer`** (1 nodes): `AdminSettingService`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Miscellaneous Module`** (2 nodes): `_payload()`, `TestAdminCouponApi`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Token Verification`** (2 nodes): `HTMLParser`, `_RichTextSanitizer`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `User Management`** (1 nodes): `AdminRoleService`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Room & Seat Models`** (1 nodes): `TestSeatAPI`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Redis Connection`** (1 nodes): `TestCourseModel`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Teacher Management`** (2 nodes): `Integration tests for city APIs.`, `TestCityAPI`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Database Seed Data`** (2 nodes): `Unit tests for the City model.`, `TestCityModel`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (2 nodes): `StudyRoom.room_type 字段验证。`, `TestStudyRoomRoomType`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `TestTeacherModel`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 220`** (1 nodes): `useCityStore`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `create_users_table  Revision ID: 0a2b2652445d Revises:  Create Date: 2026-04-17`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `create_users_table  Revision ID: 1dfa25667f22 Revises: 0a2b2652445d Create Date:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `create_activities_table  Revision ID: ba64420678cf Revises: 1dfa25667f22 Create`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `create_seats_and_bookings  Revision ID: de5e1f080747 Revises: ba64420678cf Creat`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `booking seat table  Revision ID: 2c56c35e7075 Revises: de5e1f080747 Create Date:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `create_booking_order_table  Revision ID: 985785a787d8 Revises: 2c56c35e7075 Crea`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `add_cities_table_and_room_city_id  Revision ID: 7c9d2e4f6a1b Revises: f836feddaf`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `add_booking_payment_query_schedule  Revision ID: f6a1b2c3d4e5 Revises: e5f6a1b2c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `add_notification_tables  Revision ID: a7b8c9d0e1f2 Revises: f6a1b2c3d4e5 Create`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `add room follows  Revision ID: c9d0e1f2a3b4 Revises: b8c9d0e1f2a3 Create Date: 2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `add_activity_coupon_campaign  Revision ID: f1a2b3c4d5e6 Revises: d0e1f2a3b4c5 Cr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `create_schedule_HotCourseItem  Revision ID: fccf087f0f34 Revises: e3f4a5b6c7d8 C`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `create_course_table  Revision ID: c84abd1322d4 Revises: c4d5e6f7a8b9 Create Date`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `add teacher bio and student_count  Revision ID: e7f8a9b0c1d2 Revises: c84abd1322`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `add full_custom_price to courses  Revision ID: 5d8e53290b12 Revises: a1b2c3d4e5f`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `create course_schedules table and migrate fields from courses  Revision ID: b1c2`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (1 nodes): `__APP_INFO__`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `WindowSizeOptions`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `Battery`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (2 nodes): `CourseResponse`, `将逗号分隔字符串解析为列表，None 或空字符串返回空列表`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 247`** (2 nodes): `get_active_cities()`, `Return active cities ordered by sort_order ascending.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (2 nodes): `appThemeList`, `setting`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `Tests for admin wallet API routes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 252`** (1 nodes): `TestStudyRoomAPI`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 257`** (1 nodes): `BasicProps`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 259`** (1 nodes): `directive`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 260`** (1 nodes): `RoleEnum`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (1 nodes): `animates`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 268`** (1 nodes): `DynamicProps`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 270`** (1 nodes): `params`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Settings` connect `Room Management Tests` to `Booking Payment Tests`, `Booking Service Tests`, `Auth Service Layer`, `Data Models & Schemas`, `Room & Seat Service`, `Activity Management`, `Auth & Login Tests`, `Teacher Management Tests`, `Teacher Management Tests`, `Booking Payment Tests`, `Room & Seat Management`, `Room Management Tests`, `Booking Cancellation Tests`, `Teacher Management Tests`, `Teacher Management`, `Coupon Service Layer`, `Auth & WeChat Integration`, `Payment & Wallet`, `Token Verification`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `StudyRoom` connect `Wallet Service Tests` to `Auth & Login Tests`, `Booking Cancellation Tests`, `JWT Token Tests`, `Payment & Wallet`, `Booking Cancellation Tests`, `Room Management Tests`, `Auth & Login Tests`, `SMS & Captcha Tests`, `User Security Tests`, `Course Management`, `Room & Seat Management`, `Booking Cancellation Tests`, `Booking Payment Tests`, `User & Auth Models`, `Auth & WeChat Integration`, `Community 252`, `Auth & Login Tests`, `Room & Seat Models`, `Auth & Login Tests`, `RBAC Data Models`, `Booking Cancellation Tests`, `Wallet Service Layer`, `Data Models & Schemas`, `Miscellaneous Module`, `Booking Service Tests`?**
  _High betweenness centrality (0.080) - this node is a cross-community bridge._
- **Why does `Base` connect `JWT Token Tests` to `JWT Token Tests`, `Booking Payment Tests`, `Booking Service Tests`, `Coupon System`, `Wallet Service Tests`, `Integration Tests`, `Booking Cancellation Tests`, `SMS & Captcha Tests`, `Auth & Login Tests`, `Coupon System`, `Auth & Login Tests`, `Miscellaneous Module`, `Notification System`, `Teacher & Course Models`?**
...
- **What connects `CourseScheduleItem`, `CourseListResult`, `CourseCreateParams` to the rest of the system?**
