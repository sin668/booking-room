...
系统 SHALL 在用户注册或登录成功后签发 JWT Access Token（15 分钟有效期）和 Refresh Token（7 天有效期）。用户数据从统一的 `users` 表中查询，不再按 `user_type` 过滤。注册成功后自动分配 `app_register_user` 默认角色。
...
- **GIVEN** 用户已注册，用户名 "admin01"，密码 "Abc123456"
