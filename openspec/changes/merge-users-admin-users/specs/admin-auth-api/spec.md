...
系统 SHALL 提供 `POST /api/v1/admin/auth/login` 接口，允许后台管理员使用用户名和密码登录。管理员用户数据从统一的 `users` 表中查询（`user_type='admin'`）。

#### Scenario: Successful admin login
- **WHEN** 管理员提交正确的 `username` 和 `password`
- **THEN** 系统从 `users` 表查询 `user_type='admin'` 且 `username` 匹配的用户，验证密码
- **AND** 系统返回 HTTP 200
...
系统 SHALL 提供 `GET /api/v1/admin/auth/me` 接口，返回当前登录管理员资料、角色和权限列表。管理员数据从统一的 `users` 表中查询。
...
系统 SHALL 提供 `PUT /api/v1/admin/auth/profile` 接口，允许当前管理员更新个人资料。更新操作在统一的 `users` 表上执行。

#### Scenario: Successful profile update
- **WHEN** 管理员提交 `nickname`、`email`、`mobile` 或 `avatar`
- **THEN** 系统更新 `users` 表中对应管理员记录并返回更新后的资料
...
系统 SHALL 提供 `PUT /api/v1/admin/auth/password` 接口，允许当前管理员修改密码。密码更新在统一的 `users` 表上执行。

#### Scenario: Successful password update
- **WHEN** 管理员提交正确的 `old_password`、合法的 `new_password` 和一致的 `confirm_password`
- **THEN** 系统更新 `users` 表中该管理员的 `password_hash` 并返回 HTTP 200
...
系统 SHALL 在管理接口上执行接口级权限校验。管理员上下文从统一的 `users` 表构建。

#### Scenario: Super admin bypasses permission checks
- **GIVEN** 当前用户 `user_type='admin'` 且 `is_super_admin=true`
- **WHEN** 调用任意管理接口
- **THEN** 系统允许访问
