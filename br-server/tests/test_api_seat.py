...
class TestSeatAPI:
    ...
    @pytest.mark.asyncio
    async def test_get_seat_stats_no_time_filter(self, client: AsyncClient, seed_room_and_seats):
        room_id = seed_room_and_seats
        resp = await client.get(f"/api/v1/rooms/{room_id}/seats/stats/")
        assert resp.status_code == 200
        assert resp.json() == {
            "total": 3,
            "available": 2,
    ...
    @pytest.mark.asyncio
    async def test_get_seat_stats_with_availability(self, client: AsyncClient, db_session: AsyncSession, seed_room_and_seats):
        room_id = seed_room_and_seats
        ...
        resp = await client.get(
            f"/api/v1/rooms/{room_id}/seats/stats/",
            params={"date": "2026-05-01", "start_time": "10:00", "end_time": "12:00"},
        )
        assert resp.status_code == 200
        assert resp.json() == {
            "total": 3,
    ...
    @pytest.mark.asyncio
    async def test_get_seat_stats_room_not_found(self, client: AsyncClient):
        resp = await client.get("/api/v1/rooms/9999/seats/stats/")
        assert resp.status_code == 404
        assert "not found" in resp.json()["detail"].lower()
