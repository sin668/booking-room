...
系统 SHALL 提供 `GET /api/v1/admin/bookings/` 接口，返回所有用户的预约订单分页列表。支持查询参数 `page`（默认 1）、`page_size`（默认 10，最大 50）、`status`（可选，筛选订单状态，取值为 `BookingStatus` 的五个成员值）、`room_id`（可选，筛选自习室）、`date_start`（可选，起始日期）、`date_end`（可选，结束日期）。响应包含订单详情及关联的座位和自习室信息，并包含 `booking_type`（预约类型：seat/course）、`schedule_type`（fixed/custom）与 `time_slots`（课程订单上课时间段 JSON）字段。

管理端的 `status` 筛选 SHALL 为**纯列匹配**（`bookings.status = <value>`），SHALL NOT 附加 `payment_status` 或日期派生条件。该口径与 C 端 `GET /api/v1/bookings/` 的同名派生筛选不同，属既有已知行为，本次重命名 SHALL NOT 顺手统一。

`time_slots` 字段 SHALL 遵循统一的数据契约层格式 `[{"weekday": <1-7>, "time_slot": "HH:MM-HH:MM"}]`，由 `app/utils/time_slots.py` 负责解析与构造。展示文案的格式化属前端职责，各端保持自身现状。
...
- **WHEN** 管理员发送 `GET /api/v1/admin/bookings/?status=in_progress`
- **THEN** 返回 HTTP 200，`items` 中仅包含 `status` 为 "in_progress" 的订单

#### Scenario: Admin status filter is a plain column match
- **WHEN** 管理员发送 `GET /api/v1/admin/bookings/?status=pending_start`
- **THEN** 返回 HTTP 200，`items` 中仅包含 `status` 字面等于 "pending_start" 的订单
- **AND** `status` 为 "pending_confirm" 的订单 SHALL NOT 出现在结果中
- **AND** 结果 SHALL NOT 受 `payment_status` 或开课日期影响
...
- **WHEN** 管理员发送 `GET /api/v1/admin/bookings/?status=in_progress&room_id=1&date_start=2026-05-01`
...
系统 SHALL 提供 `POST /api/v1/admin/bookings/{booking_id}/cancel` 接口，允许管理员取消任意用户的订单。`in_progress` 状态订单按通用取消策略取消；`pending_confirm` 已支付订单全额退款取消；课程预约（`booking_type='course'`）且状态为 `pending_start`/`pending_confirm` 的订单，已支付时全额退款、恢复优惠券、写退款流水，并仅删除该订单专属的 `schedule_type='custom'`（定制）的 `course_schedules` 与对应 `lesson_schedules` 记录；`schedule_type='fixed'`（固定班课）排课为课程共享资源，即使无其他订单引用也一律保留；排课仍被其他非取消订单共享时同样保留。

「课程预约且处于待开始」的判定（原 `booking_type='course' AND status IN ('pending','pending_confirm')`）SHALL 由领域层单一判定函数提供，服务端与 br-admin 前端 SHALL NOT 各自维护一份同语义的内联实现。

#### Scenario: Successful cancellation
- **WHEN** 管理员发送 `POST /api/v1/admin/bookings/1/cancel`，该订单状态为 "in_progress"
...
> 标题沿用主 spec 原名（MODIFIED 整块替换语义要求）；其中“pending”指旧订单状态值，已重命名为 `pending_start`。
- **WHEN** 管理员取消 `booking_type='course'`、状态为 "pending_start"、已支付且关联订单专属排课的订单
...
> 标题沿用主 spec 原名；“pending”指旧订单状态值 `pending_start`。
- **WHEN** 管理员取消的 `pending_start` 课程订单关联的排课仍被其他非取消订单引用
- **THEN** 订单状态变为 "cancelled" 并全额退款，但排课与课时记录保留

#### Scenario: Cancel pending fixed course booking keeps fixed schedule
> 标题沿用主 spec 原名；“pending”指旧订单状态值 `pending_start`。
- **WHEN** 管理员取消的 `pending_start` 课程订单关联的排课 `schedule_type` 为 "fixed"，且无其他订单引用
